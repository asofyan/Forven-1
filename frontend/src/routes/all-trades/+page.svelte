<script lang="ts">
	...
	// (toNumber / livePrice / liveUnrealizedUsd live in $lib/utils/livePnl,
	// shared with the global header P&L ticker.)
	...
	function tradeHref(t: ForvenTrade): string {
		...
		return `${live ? '/live-trades' : '/paper-trades'}?select=${encodeURIComponent(sid)}`;
	}
