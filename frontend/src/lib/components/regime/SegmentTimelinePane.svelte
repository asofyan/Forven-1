<script lang="ts">
	...
	function segmentColor(regime: string): string {
		return regimeSwatchClass(regime) || 'bg-slate-500/70';
	}
...
<div class="mt-3 rounded-md border border-slate-700 bg-slate-950/90 p-2">
	<div class="relative h-28 overflow-hidden rounded-md border border-slate-800 bg-slate-950">
		...
		<div class="pointer-events-none absolute inset-x-0 top-1/2 border-t border-dashed border-slate-700/80"></div>
		<div class="absolute inset-x-0 top-0 bottom-4" bind:this={chartContainer}></div>
		<div class="pointer-events-none absolute inset-x-0 bottom-0 flex h-4 overflow-hidden border-t border-slate-800/80">
			{#each segments as segment}
				<div
		...
		{#if pricePoints.length === 0}
			<div class="absolute inset-0 flex items-center justify-center bg-slate-950/55 text-xs text-slate-500">
	...
	<div class="mt-2 flex flex-wrap items-center justify-between gap-2 text-[11px] text-slate-500">
		<div class="flex items-center gap-2">
			<span class="h-[2px] w-6 rounded-full bg-slate-50"></span>
			<span>Close</span>
		</div>
		<div class="flex items-center gap-2">
			<span class="h-2 w-2 rounded-sm bg-sky-400/70"></span>
