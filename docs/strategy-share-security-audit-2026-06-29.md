...
- **R2 (out-of-process) — SANDBOX-ONLY IMPORT LANDED:** imported code is now executed **exclusively in the locked-down worker**, never the trusted parent, and **code-bundled import is re-enabled through it**. Mechanism:
  ...
  - **Remaining (functional, fail-closed — not a security gap):** a few lower-traffic construction sites are not yet proxy-aware — the optimizer's `parameter_space()` probe (`optimizer.py:1010/1041`), the scanner's ad-hoc non-`get_active` builds (`scanner.py:2726/6188`), and `preview_strategy_signals`. An imported strategy that reaches those *fails closed* (no code runs in the parent) but can't complete that step, so full-gauntlet optimization/WFA and the ad-hoc live path for imported strategies need the same proxy wiring as a follow-up. Import + quick_screen backtest + paper hydration are wired.
