...
def test_hypothesis_verdict_columns_exist(forven_db):
    with get_db() as conn:
...
def test_hypothesis_verdict_memos_table_exists(forven_db):
    with get_db() as conn:
...
def test_hypothesis_verdict_memos_index_exists(forven_db):
    with get_db() as conn:
        rows = conn.execute(
