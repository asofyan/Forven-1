...
def test_discover_happy_path(monkeypatch):
    ...
    with patch("forven.research_sources.github.search_github_repos") as m:
...
def test_discover_pat_from_registry_passed_to_client(monkeypatch):
    ...
    captured = {}
    with patch("forven.research_sources.github._client") as cl, patch("forven.research_sources.github.search_github_repos") as sr:
...
def test_inspect_happy_path(monkeypatch):
    _with_registry(monkeypatch, {"orgs": ["quantopian"], "rate_limit_per_min": 60, "personal_access_token": None})
    with patch("forven.research_sources.github.inspect_github_repo") as m:
