...
from forven import brain_memory, brain_inject
from forven.diagnostics import check_brain_cache_hit_rate


def _set_memory(forven_db, body: str) -> None:
    brain_memory.set_memory(body, mutated_by="test")


def test_empty_memory_still_emits_well_formed_fence(forven_db):
...
def test_user_message_places_fence_before_situational_text(forven_db):
...
def test_extract_leading_user_text_stops_at_boundary(forven_db):
    msg = brain_inject.build_user_message(
...
def test_consecutive_cycles_with_identical_memory_share_leading_text(forven_db):
    _set_memory(forven_db, "stable memory body")
...
def test_prompt_hash_stable_when_memory_unchanged(forven_db):
    _set_memory(forven_db, "stable")
...
def test_prompt_hash_changes_when_memory_mutates(forven_db):
    _set_memory(forven_db, "first")
    msg_a = brain_inject.build_user_message("x", brain_inject.get_memory_body_for_injection())
    _set_memory(forven_db, "second")
...
def test_prompt_hash_changes_when_system_changes(forven_db):
    _set_memory(forven_db, "stable")
...
def test_record_cache_observation_counts_hits_and_misses(forven_db):
    h1 = "a" * 64
    h2 = "b" * 64
...
def test_diagnostics_check_reports_rate_after_two_cycles(forven_db):
    _set_memory(forven_db, "a")
...
def test_diagnostics_check_passes_with_no_history(forven_db):
    res = check_brain_cache_hit_rate()
    assert res.status == "pass"
    assert res.detail.get("comparisons") == 0


def test_fence_constants_are_unique_and_searchable(forven_db):
