...
def _client():
    # forven.api_core does not expose a FastAPI `app`; the settings routes live
    # on forven.routers.system, which forven.api includes into the real app.
    # Importing forven.api pulls in uvicorn + the full middleware stack (and
...
def test_default_setup_wizard_completed_at_is_null(forven_db):
    client = _client()
...
def test_put_ui_section_sets_completed_at(forven_db):
    client = _client()
...
def test_put_ui_section_clears_completed_at_when_null(forven_db):
    client = _client()
...
def test_put_ui_section_rejects_non_string(forven_db):
    client = _client()
