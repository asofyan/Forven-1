...
from forven.db import get_db, kv_get as kv_get, kv_set
...
log = logging.getLogger("forven.simulation")
...
class SimulationRunner:
    ...
    async def run(self):
        ...
        kv_set("forven:simulation:active", True)
        kv_set("forven:simulation:exec_mode", self.config.exec_mode)

        from forven.sim.clock import set_sim_active
        set_sim_active(True)
        ...
        try:
            ...
            from forven.sim.data_pump import prefetch_candles
            ...
            for i, bar_time in enumerate(bar_timestamps):
                ...
                from forven.sim.clock import set_sim_time
                ...
                from forven.regime import invalidate_cache
                invalidate_cache()
    ...
    def _finalize(self):
        ...
        kv_set("forven:simulation:exec_mode", None)
        kv_set("forven:simulation:time", None)

        # Clear data pump cache
        try:
            from forven.sim.data_pump import clear_cache
            clear_cache()
        except ImportError:
            pass
