...
from forven.strategies.backtest import expand_strategy_trade_modes, validate_backtest_risk_controls
...
log = logging.getLogger("forven.lab_strategy_pool")
...
def get_main_db_readonly() -> sqlite3.Connection:
    ...
    db_path = Path(cfg.FORVEN_DB).expanduser().resolve()
...
def inspect_strategy_pool(
    *,
    ...
    if not Path(cfg.FORVEN_DB).exists():
        return {
...
def _persist_quarantine_events(skipped: list[dict[str, Any]]) -> None:
    ...
    """
    try:
        from forven.lab_db import create_selection_event, existing_quarantine_event_keys
