...
def build_server(client: ForvenClient | None = None) -> FastMCP:
    ...
    server = FastMCP(
        name="forven",
        instructions=(
            "Forven AI Drop Zone — create, register, and backtest trading "
            "strategies. Start by calling forven_get_context to learn the "
            "system, then (optionally) forven_create_session to group your "
            "work, write strategy .py files to the workspace, register them "
            "with forven_register_strategy_file, and backtest with "
            "forven_run_backtest. Use forven_promote_strategy and "
            "forven_get_gate_report to inspect lifecycle gates without "
            "bypassing them."
        ),
    )
    ...
    def forven_get_context() -> dict[str, Any]:
        return forven.get("/api/ai-dropzone/context")
    ...
    def forven_get_session(session_id: str) -> dict[str, Any]:
        return forven.get(f"/api/ai-dropzone/sessions/{session_id}")
    ...
    @server.tool(
        ...
        description=(
            ...
            "Use after forven_run_backtest to inspect outcomes."
        ),
    )
    ...
    def forven_close_session(session_id: str) -> dict[str, Any]:
        return forven.post(f"/api/ai-dropzone/sessions/{session_id}/close")
    ...
    def forven_run_verdict(
        ...
        if tests:
            body["tests"] = tests
        return forven.post("/api/backtesting/verdict/run", body)
    ...
    @server.tool(
        ...
        description=(
            ...
            "does not promote; use forven_promote_strategy to attempt paper."
        ),
    )
    ...
    def forven_run_gauntlet_candidate(
        ...
        verdict = None
        if run_verdict:
            verdict = forven_run_verdict(strategy_id, dataset_id, verdict_tests)
