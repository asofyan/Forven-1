"""Thin auth-aware HTTP client for hitting the running Forven API.

The MCP server is an external process and must cross the wire rather than
reach into forven internals — the user might be running the backend with a
...
class ForvenClient:
    """Blocking httpx client bound to a single Forven backend."""
    ...
    def __init__(
        ...
        try:
            self.timeout = float(timeout if timeout is not None else os.environ.get("FORVEN_MCP_TIMEOUT") or 60.0)
