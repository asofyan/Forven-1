...
from forven.routers import agents as agents_routes
from forven.routers import auth as auth_routes
from forven.routers import strategies as strategies_routes
...
def apply_legacy_response_headers(response: Response, route_path: str) -> None:
    ...
    response.headers["X-Forven-Legacy-Route"] = route_path
    log.warning(
...
def legacy_forven_get(legacy_path: str, request: Request, limit: int = 50):
    """Legacy compatibility layer for older callers using `/api/forven/*`."""
    ...
    raise HTTPException(status_code=404, detail=f"Legacy endpoint not supported: /api/forven/{normalized_path}")


def post_brain_chat_legacy(body):
    from forven.routers import system as system_routes
