...
machine-specific path (``/home/trestor/.forven/workspace/agents/execution-trader``),
which raised ``ModuleNotFoundError`` on every other machine and silently disabled the
safeguard layer. It now lives in ``forven.agents`` and is imported normally.
...
which regime* is handled upstream in ``forven.regime``; this is a last-line defence on
...
class MarketRegime(str, Enum):
    ...
    aliases understood by :func:`forven.regime.normalize_regime_label`
